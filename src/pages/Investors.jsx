...
import { getInvestors } from '../services/investorApiClient';
...
const Investors = () => {
  ...
  const loadInvestors = useCallback(async () => {
    ...
    try {
      ...
      setInvestors((result.items || []).map((inv, idx) => ({ ...inv, id: inv.id || idx + 1 })));
  ...
  const columns = [
    { field: 'name', headerName: 'Obligor Name', flex: 1.5, minWidth: 200 },
  ...
  return (
    <div className="space-y-6">
      ...
      >
        <div>
          <h2 className="text-2xl font-bold text-gray-100">Investors</h2>
          <p className="text-gray-400 mt-1">
            Investor data loaded from PostgreSQL
          </p>
        </div>
        <Stack direction="row" spacing={1}>
          <Chip label={`${totalCount.toLocaleString()} total`} color="primary" variant="outlined" size="small" />
          <Button size="small" variant="outlined" startIcon={<RefreshCw size={14} />} onClick={loadInvestors}>
            Refresh
          </Button>
